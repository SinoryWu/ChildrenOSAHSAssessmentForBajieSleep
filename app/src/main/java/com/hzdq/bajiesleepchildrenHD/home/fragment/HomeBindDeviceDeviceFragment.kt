...
class HomeBindDeviceDeviceFragment : Fragment() {
...
//                binding.userFragmentRecyclerView.scrollToPosition(0)
            ...
            override fun onItemClick(sn: String, status: Int,itemView:View) {
                when(status){
                    1 -> {
                        ...
                        Navigation.findNavController(itemView).navigate(R.id.action_homeBindDeviceDeviceFragment_to_homeBindDeviceUserFragment)
                    }
                    else -> {
        ...
        binding.choiceDeviceSearch.addTextChangedListener(object :TextWatcher{
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                lifecycleScope.launch {
                    delay(200)
                    shp.saveToSp("homebinddevicedevicekeyword",s.toString())
                    delay(200)
//                    homeBindDeviceViewModel.keywords.value = s.toString()
                    homeBindDeviceDeviceAdapter.notifyDataSetChanged()
                    homeBindDeviceViewModel.resetHomeDeviceDeviceQuery()
