...
class RecoveryDeviceDialog(context: Context, confirmAction:ConfirmAction): Dialog(context, R.style.CustomDialog)  {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val view: View =
            LayoutInflater.from(context).inflate(R.layout.dialog_recovery_device, null)
        setContentView(view)
        window!!.setLayout(screenWidth,screenHeight )

        val cancel = view.findViewById<TextView>(R.id.recovery_device_cancelBtn)
        val confirm = view.findViewById<TextView>(R.id.recovery_device_submitBtn)

        cancel.setOnClickListener {
            dismiss()
        }
        confirm.setOnClickListener {
            confirmAction?.onRightClick(it)
            dismiss()
        }
    }
